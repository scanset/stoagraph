...
func main() {
	...
	crossingBudget := flag.Int("crossing-budget", 0, "max FORWARDED crossings per agent session (0 = unlimited); enforces the per-session leakage cap N (Planning/34)")
	requireBounded := flag.Bool("require-bounded", false, "refuse to bind any recipe with UNBOUNDED leakage (a free-text field reaching an external sink); high-assurance posture")
	flag.Parse()
